...
export const Switch: React.FC<SwitchProps> = ({ id, checked = false, autoFocus, onChange, onBlur, hasError = false, disabled = false, hasWarning }) => {
  ...
  return (
    <SwitchRadix.Root
      ...
      className="p-1 w-12 h-7 rounded-full bg-background cursor-pointer flex items-center justify-start hover:opacity-100 focus:ring-1 ring-primary disabled:opacity-70 disabled:cursor-default disabled:pointer-events-none data-[warning=true]:ring-yellow-500 data-[warning=true]:ring-1 data-[error=true]:ring-red-500 data-[error=true]:ring-1"
    >
      <SwitchRadix.Thumb
